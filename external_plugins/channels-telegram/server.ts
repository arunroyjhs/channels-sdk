...
telegram.onMessage(async (msg) => {
  // /start — auto-pair immediately. First user to DM gets connected, zero friction.
  if (msg.command === '/start') {
    if (pairing.isAuthorized(msg.chatId, msg.userId)) {
      await telegram.send({
        chatId: msg.chatId,
        text: '✅ Already connected! Send a message and your agent will respond.',
        keyboard: [[
    ...
    // Auto-pair — no codes, no deep links, just connect
    const device = pairing.autoApproveDeepLink(
      msg.chatId, msg.userId, msg.username, msg.username,
    );
    await telegram.send({
      chatId: msg.chatId,
      text: `✅ Connected!\n\nYou're now linked to your Claude Code agent. Just send a message — it will respond here.\n\nType /help to see what I can do.`,
      keyboard: [[
        { text: '📊 Status', callbackData: 'cmd:status' },
        { text: '❓ Help', callbackData: 'cmd:help' },
      ]],
  ...
  // Check authorization — auto-connect on first message if no devices exist yet
  if (!pairing.isAuthorized(msg.chatId, msg.userId)) {
    if (pairing.getDevices().length === 0) {
      // No one connected yet — auto-approve first user
      pairing.autoApproveDeepLink(msg.chatId, msg.userId, msg.username, msg.username);
      await telegram.send({
        chatId: msg.chatId,
        text: '✅ Auto-connected! Your message is being processed.',
        silent: true,
      });
    } else {
      await telegram.send({
        chatId: msg.chatId,
        text: '🔒 Not authorized. Ask the bot owner to add you, or send /start.',
      });
      return;
    }
  }
